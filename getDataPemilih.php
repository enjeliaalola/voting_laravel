<?php
	...
	$getDataQuery = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pemilih {$where} ORDER BY nama ASC");
	while($getDataView = mysqli_fetch_array($getDataQuery)){
		$dataJsonGenerate[] = array(
			"id" => $getDataView['id'],
			"nama" => $getDataView['nama'],
			"nisn" => $getDataView['nisn'],
			"kelas" => $getDataView['kelas'],
			"username" => $getDataView['username'],
			"password" => $getDataView['password_backup'],
